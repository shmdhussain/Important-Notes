	...
	function fnDocReady(){
	 ...
	 function fnShowSlow() {
		 ...
		 fnRunSlides(allslides);
		 //sl=setInterval(fnShowSlow,3000);
	 }
	 function fnRunSlides(allslides) {
		 console.log("running");
		 for (i=0;i<=allslides.length-1;i++){
			doTimer(allslides[i], 3000);
		 }
	 }
	function doTimer(itemObj, delay) {
	  setTimeout(function() {console.log(itemObj);}, delay);
	}
	
	function fnDelay(allslides,i) {
		console.log(allslides+" "+i);
		if(allslides.length!=i){
			console.log(allslides[i]);
		}
